...
def header_footer(canvas, doc):
    ...
    canvas.drawRightString(W - 15*mm, H - 19*mm,
        f"AML AE26 ChainDrive 04 spring_update | 2026-06-19")
    canvas.setFillColor(DARK_BLUE_CL)
...
def build_pdf_section(fig_paths, summary_rows, out_path, speeds):
    ...
    interp_text = (
        ...
        f"<b>{MODEL}</b> model covers {len(speeds)} speed points "
        f"({', '.join(str(s) for s in speeds)} rpm) "
        "over 15 cam cycles (5400° cam angle) each.  The results are evaluated for pump-up in the "
    ...
    story.append(Paragraph(
        ...
        f"{MODEL}.etd. "
        f"Results auto-generated by analyze_hlif_pumpup.py — 2026-06-19.",
        st["note"]))
