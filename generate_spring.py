...
# Oval wire cross-section: compute actual axial extent for information only.
# wire_a=2.92mm (drawing value) is used for coil pitch regardless of oval shape —
# Abaqus CAE meshing (unlike Gmsh) handles touching coil surfaces correctly.
# L0 stays at the drawing value of 46.1mm.
if WIRE_PROFILE == "oval":
    _t_ov = _np.linspace(0, 2 * _np.pi, 2000)
    _y_ov = OVAL_B * _np.cos(_t_ov) * _np.exp(OVAL_C * (wire_r / 2) * _np.sin(_t_ov))
    _wire_a_actual = 2.0 * float(_np.max(_np.abs(_y_ov)))
    print(f"  Oval wire axial extent: {_wire_a_actual:.3f} mm  (drawing wire_a={wire_a} mm used for pitch)")
